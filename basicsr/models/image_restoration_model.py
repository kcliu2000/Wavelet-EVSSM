...
class ImageRestorationModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('edge_loss_opt'):
            edge_type = train_opt['edge_loss_opt'].pop('type')
            cri_edge_cls = getattr(loss_module, edge_type)
            self.cri_edge = cri_edge_cls(**train_opt['edge_loss_opt']).to(
                self.device)
        else:
            self.cri_edge = None
    ...
    def optimize_parameters(self, current_iter, tb_logger):
        ...
        # perceptual loss
        if self.cri_fft:
            l_fft = self.cri_fft(preds[-1], gt1)
            l_total += l_fft
...
########################################
        ...
        # perceptual loss
        if self.cri_fft:
            l_fft = self.cri_fft(preds[-1], gt2)
            l_total += l_fft
            loss_dict['l_fft'] = l_fft
